...
namespace bs\phpSysInfo\components;

use Yii;
use yii\base\Component;

class SystemInformationProvider extends Component
{
    /**
    ...
    {
        return $this->_sys->getKernel();
    }

    public function getDistribution()
    {
        return $this->_sys->getDistribution();
    }
    ...
    public function getLoadPercent()
    {
        if ($this->_sys->getLoadPercent() !== null) {
            return $this->_sys->getLoadPercent();
        } elseif (PSI_OS === 'Darwin') {
            $usage = $this->getMacLoadPercent();
            return $usage[0]*100;
        } else {
            $avg = $this->_sys->getLoad();
            $avg_a = explode(' ', $avg);
            $cpus = $this->_sys->getCpus();
            if ($cpus) {
                return ($avg_a[0]*100)/count($cpus);
            }
        }
    ...
    {
        return $this->getSwapTotal()-$this->getSwapUsage();
    }
    ...
    {
        return $this->_sys->getUptime();
    }
    /**
     * @return string
     */
    public function getUptimeString()
    {
        $seconds = $this->_sys->getUptime();
        $d = intval($seconds/(3600*24));
        $h = intval($seconds/3600)-$d*24;
        $m = intval($seconds/60)-$d*24*60-$h*60;
        $s = $seconds%60;
    ...
    {
        /** @var \DiskDevice[] $devices */
        $devices = $this->_sys->getDiskDevices();
        $result = 0;
        foreach ($devices as $device) {
            $result += $device->getTotal();
        }
    ...
    {
        return $this->getStorageTotal()-$this->getStorageUsage();
    }
    ...
    {
        $psiRoot = Yii::getAlias('@vendor/phpsysinfo/phpsysinfo');
        ...
        require($include.'/to/class.System.inc.php');
        require($include.'/to/device/class.CpuDevice.inc.php');
        require($include.'/to/device/class.DiskDevice.inc.php');
        require($include.'/to/device/class.HWDevice.inc.php');
        require($include.'/to/device/class.NetDevice.inc.php');
        require($include.'/to/device/class.SensorDevice.inc.php');
        require($include.'/to/device/class.UPSDevice.inc.php');

        require($include.'/interface/class.PSI_Interface_OS.inc.php');

        require($include.'/os/class.OS.inc.php');
        require($include.'/os/class.BSDCommon.inc.php');
        require($include.'/os/class.Linux.inc.php');
        require($include.'/os/class.FreeBSD.inc.php');
        require($include.'/os/class.Darwin.inc.php');

        require($include.'/os/class.AIX.inc.php');
        require($include.'/os/class.Android.inc.php');
        require($include.'/os/class.DragonFly.inc.php');
        require($include.'/os/class.Haiku.inc.php');
        require($include.'/os/class.HPUX.inc.php');
        require($include.'/os/class.Minix.inc.php');
        require($include.'/os/class.NetBSD.inc.php');
        require($include.'/os/class.OpenBSD.inc.php');
        require($include.'/os/class.QNX.inc.php');
        require($include.'/os/class.SunOS.inc.php');
        require($include.'/os/class.WINNT.inc.php');
    }
    ...
    private function readConfig()
    {
        if (!defined('PSI_CONFIG_FILE')) {
             ...
             */
            define('PSI_CONFIG_FILE', Yii::getAlias($this->configFilePath));
